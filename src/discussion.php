...
<body>
    ...
    if(!is_null($_SESSION['logged_username'])){
        ...
        $profile = $row['stud_profile_picture'];
        $id = $row['stud_id'];
    }elseif(!is_null($_SESSION['logged_teacher'])){
        include("teacher-nav.php");
        $username = $_SESSION['logged_teacher'];
        //Sql to find the teac_id
        $sql = "SELECT * FROM teacher WHERE teac_username = '$username'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $profile = $row['teac_profile_picture'];
        $id = $row['teac_id'];
    }
    ...
    <section class="discussion-section">
        ...
        <section class="ds-top">
            ...
            <div class="ds-post-question">
                <form action="discussion-post.php?qid=<?php echo $qid;?>&cid=<?php echo $cid?>" method="POST">
                    <input type="hidden" name="user-id" value="<?php echo $id?>">
                    <div class="ds-post-poster">
                        <img src="Images/<?php echo $profile?>" alt="">
                        <div class="ds-post-ta">
                            <textarea name="answer-reply" id="" cols="30" rows="10" placeholder="Type your answer here..." required></textarea>
                            <input type="submit" class="ds-post-btn" value="Post" name="add-answer">
                        </div>
                    </div>
                </form>
            </div>
