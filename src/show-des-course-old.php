...
$sql = "SELECT * ,(SELECT AVG(rate_course_value) FROM rating r WHERE (r.cour_id = c.cour_id)) AS avg_rating FROM course c, teacher t WHERE (c.teac_id = t.teac_id) AND c.cour_category = 'Design'";
...
if (isset($_POST['search'])) {
    $search_query ="SELECT * ,(SELECT AVG(rate_course_value) FROM rating r WHERE (r.cour_id = c.cour_id)) AS avg_rating FROM course c, teacher t WHERE (c.teac_id = t.teac_id) AND c.cour_name LIKE '%".$_POST['search']."%' AND c.cour_category = 'Design'";
    $result = executeQuery($search_query);
    $starting_num = 0;
} else {
    //Retreive selected result from the table and display them on page
    $sql = "SELECT * ,(SELECT AVG(rate_course_value) FROM rating r WHERE (r.cour_id = c.cour_id)) AS avg_rating FROM course c, teacher t WHERE (c.teac_id = t.teac_id) AND c.cour_category = 'Design' ORDER BY RAND() LIMIT " . $starting_num . ',' . $result_per_page;
